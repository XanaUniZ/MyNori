...
class SingleScatteringLens : public Integrator {
public:
    SingleScatteringLens(const PropertyList& props) {
    ...
    Color3f GetLiSingleScattering(const Scene* scene, Sampler* sampler, const Ray3f& ray) const {
        ...
        // float focalDistance = 5.5;
        // // Transform m_cameraToWorld = propList.getTransform("toWorld", Transform());
        // // Ray3f sampledRay;
        // // <<Sample point on lens>> 
        // Point2f pLens = lensRadius * Warp::squareToUniformDisk(sampler->next2D());

        // // <<Compute point on plane of focus>> 
        // float ft = abs(focalDistance / (ray.d.z()));
        // // cout << "ft: " << ft << endl;
        // // cout << "ray.d.z(): " << ray.d << endl;
        // Point3f origin = Point3f(pLens.x(), pLens.y(), 0);
        // Point3f pFocus = ft * ray.d;

        // // <<Update ray for effect of lens>> 
        // // sampledRay.o = Point3f(pLens.x(), pLens.y(), 0);
        // // sampledRay.o = ray.o;
        // // sampledRay.d = (pFocus - sampledRay.o).normalized();
        // // Point3f origin = Point3f(pLens.x(), pLens.y(), 0);
        // Vector3f direction = (pFocus - origin).normalized();
        // // cout << "sampledRay.o: " << pLens.x() << " " << pLens.y() << endl;
        // // cout << "sampledRay.d: " << direction << endl << endl;
        // Ray3f sampledRay = Ray3f(origin + ray.o, direction);

        Ray3f sampledRay = ray;
        // cout << "ray.o: " << ray.o.x() << " " << ray.o.y() << " "  << ray.o.z() << endl;
        // cout << "sampledRay.o: " << sampledRay.o.x() << " " << sampledRay.o.y() << " "  << sampledRay.o.z() << endl;
        // cout << "sampledRay.d: " << direction << endl << endl;
        ...
        float sigma_s = 0.0f;   // Coeficiente de scattering
        ...
        Intersection its;
        bool has_interseced = scene->rayIntersect(sampledRay, its);
        if (!has_interseced){
            // Lo += std::exp(-sigma_t * t) * scene->getBackground(sampledRay);
            // maxDistance = 10;
        }

        else if(its.mesh->isEmitter()){
            Color3f direct = DirectRadiance(&its, &sampledRay.o);
            ...
            Lo += std::exp(-sigma_t * its.t) * EmitterSampling(&its, sampledRay, scene, sampler);
            // Lo += EmitterSampling(&its, sampledRay, scene, sampler);
            maxDistance = its.t;
        }

        // while (t < maxDistance) {
        //     // Punto de muestreo actual
        //     Point3f samplePoint = t*sampledRay.d + sampledRay.o;
        //     t += stepSize;          
...
NORI_REGISTER_CLASS(SingleScatteringLens, "single_scattering_lens");
