...
public:
    ThinLensCamera(const PropertyList &propList) {
        ...
        lensRadius = propList.getFloat("lensRadius", 0.0);
        focalDistance = propList.getFloat("focalDistance", 0.0);
    }
    ...
    Color3f sampleRay(Ray3f &ray,
        ...
        Point2f randomSample;
        randomSample[0] = ((double) rand() / (RAND_MAX)) + 1;
        randomSample[1] = ((double) rand() / (RAND_MAX)) + 1;
        Point2f pLens = lensRadius * Warp::squareToUniformDisk(randomSample);

        // <<Compute point on plane of focus>> 
        float ft = abs(focalDistance / d.z());
        ...
        float invZ = 1.0f / direction.z();

        ray.o = m_cameraToWorld * origin;
        ray.d = m_cameraToWorld * direction;
